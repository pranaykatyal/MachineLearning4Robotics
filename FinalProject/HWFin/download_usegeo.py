...
async def process_folder(page: Page, folder_path: str, root_dir: str):
    ...
    # Recurse folders
    # -----------------------
    SKIP_FOLDERS = ["undistorted_images"]  # Skip images (we have them), but download Depth_resized!
    
    for folder in folders:
        # Skip folders we already have
        if folder in SKIP_FOLDERS:
            log("INFO", f"SKIPPING folder (already have from professor): {folder}")
            continue
...
if __name__ == "__main__":
    # Download all 3 datasets (camera_poses and camera_intrinsics.txt only)
