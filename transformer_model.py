...
class Transformer(nn.Module):
    ...
    # Modularized into encode/decode functions for optimizing the decoding/translation process
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, trg_representations_batch, src_representations_batch, trg_mask, src_mask):
        # Define anonymous (lambda) function which only takes trg_representations_batch (trb)
...
if __name__ == "__main__":
    ...
    batch_size = 32
    src_sentence_lenght = 25
    trg_sentence_lenght = 27
    src_token_ids_batch = torch.randint(1, 20000, size=(batch_size, src_sentence_lenght))
    trg_token_ids_batch = torch.randint(1, 24000, size=(batch_size, trg_sentence_lenght))
